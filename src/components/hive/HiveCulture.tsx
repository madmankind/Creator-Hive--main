...
function StoryImage({ src, alt, className, sizes, priority, fill }: {
  ...
  const fallbackSrc = FALLBACKS[0];
  const onImgError = () => {
    if (safeSrc !== fallbackSrc) setSafeSrc(fallbackSrc);
  };

  const isExternal = !safeSrc.includes("unsplash.com") && !safeSrc.includes("creatorhive.ae");
  if (isExternal) {
  ...
  return <Image src={safeSrc} alt={alt} fill={fill} priority={priority} className={className} sizes={sizes} onError={onImgError} />;
}
...
const SHOWREEL: { type: SlideType; src: string; poster?: string }[] = [
  { type: "youtube", src: "https://www.youtube.com/watch?v=t9_9e4BcaAc" },
];
...
function ShowreelBg() {
  ...
  useEffect(() => {
    ...
    if (SHOWREEL[current]?.type === "youtube") return;
    if (paused) return;
  ...
  const youtubeVideoId = isYT ? ytId(slide.src) : "";
      ...
      {isYT && (
        <>
          {!ytFailed && (
            <iframe
              key={`yt-${current}`}
              title="Creator Hive spotlight video"
              src={`https://www.youtube.com/embed/${youtubeVideoId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${youtubeVideoId}&rel=0&modestbranding=1&iv_load_policy=3&playsinline=1`}
...
const SPOTLIGHT: {
  ...
  talentName: "Irene",
  talentHref: "/creators/talent-irene",
  credits: "",
  cta: { text: "Explore talent →", href: "/dashboard/hive" },
  isPaid: false,
};
...
export function HiveCulture({ dbStories = [], activeCategory = "Global" }: Props) {
  ...
  const [readerStory, setReaderStory] = useState<Card | null>(null);
  ...
  const openReader = useCallback((story: Card) => {
    ...
    <div className="w-full pb-10">
      ...
      <section className="relative left-1/2 mb-10 w-screen max-w-[100vw] -translate-x-1/2">
        <div className="relative aspect-[21/9] min-h-[240px] w-full overflow-hidden sm:aspect-[21/8] lg:aspect-[3.2/1]">
          ...
          {/* Spotlight content */}
          <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col items-start justify-end px-6 pb-3 sm:px-8 sm:pb-4 lg:px-10 lg:pb-5">
            ...
            {SPOTLIGHT.talentName ? (
              SPOTLIGHT.talentHref ? (
                <Link
                  href={SPOTLIGHT.talentHref}
                  className="max-w-2xl text-[clamp(1.8rem,3.8vw,2.8rem)] font-semibold leading-[1.04] tracking-[-0.03em] text-white underline decoration-white/30 underline-offset-4 transition hover:text-white/85 hover:decoration-white/55"
                >
                  {SPOTLIGHT.talentName}
                </Link>
              ) : (
            ...
            {SPOTLIGHT.cta && (
              <Link href={SPOTLIGHT.cta.href}
                className="inline-flex w-fit items-center gap-2 rounded-full px-5 py-2.5 text-[12px] font-semibold transition-all"
      ...
      {hasContent ? (
        <>
              ...
              <div ref={scrollRef}
                ...
                {trending.map((t) => (
                  <button
                    key={t.id}
                    type="button"
                    onClick={() => openReader(t)}
                    className="w-[280px] shrink-0 snap-start text-left sm:w-[300px] lg:w-[320px]"
                  >
                    <article className="group overflow-hidden rounded-lg border border-white/[0.06] bg-white/[0.02] transition hover:border-white/[0.12]">
                      <div className="relative aspect-[16/10] w-full overflow-hidden bg-stone-900/20">
                        ...
                        <div className="absolute right-0 bottom-0 h-full w-full bg-gradient-to-t from-black/50 to-transparent" />
          ...
          <section className="mb-10">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5">
              {grid.map((s) => (
                <button key={s.id} type="button" onClick={() => openReader(s)} className="w-full text-left">
        ...
        <div className="pt-2">
          <p className="text-center text-[13px] text-white/30">No stories yet — run ingestion to populate Culture.</p>
        </div>
      )}
      ...
      {readerStory && (
        <>
          ...
          />
          <aside
            className="fixed inset-x-0 bottom-0 top-0 z-[90] overflow-hidden border-white/[0.08] bg-[#09090d] shadow-[0_20px_80px_rgba(0,0,0,0.65)] md:inset-y-0 md:right-0 md:left-auto md:w-[min(48vw,760px)] md:border-l"
            role="dialog"
            aria-modal="true"
            aria-label="Story reader"
          >
            <div className="flex h-full flex-col">
              <header className="sticky top-0 z-10 flex items-center justify-between border-b border-white/[0.07] bg-[#09090d]/92 px-4 py-3 backdrop-blur-xl sm:px-5">
                <div className="min-w-0">
                  <p className="truncate text-[10px] font-semibold uppercase tracking-[0.14em] text-amber-400/75">{readerStory.cat}</p>
                  <p className="truncate text-[11px] text-white/45">{readerStory.meta}</p>
                </div>
                <div className="ml-3 flex items-center gap-2">
                  <a
                    href={readerStory.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 rounded-full border border-white/[0.14] px-3 py-1.5 text-[11px] font-medium text-white/80 transition hover:bg-white/[0.08]"
                  >
                  ...
                  <button
                    type="button"
                    onClick={closeReader}
                    className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/[0.12] text-white/70 transition hover:bg-white/[0.08]"
                    aria-label="Close reader"
                  >
                    <X size={15} />
                  </button>
              ...
              <div className="relative flex-1 overflow-hidden">
                {readerLoading && (
                  <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#09090d]/70">
                    <p className="text-[12px] text-white/55">Loading article…</p>
                  </div>
                )}
                ...
                {!readerLoadError ? (
                  <iframe
                    key={readerStory.url}
                    src={readerStory.url}
                    title={readerStory.title}
                    className="h-full w-full border-0 bg-[#0a0a0f]"
                    referrerPolicy="no-referrer"
                    onLoad={() => setReaderLoading(false)}
                    onError={() => {
                      setReaderLoadError(true);
                      setReaderLoading(false);
                    }}
                  />
                ) : (
                  <div className="scrollbar-none h-full overflow-y-auto px-4 pb-7 pt-4 sm:px-5">
                    <div className="relative mb-4 aspect-[16/10] w-full overflow-hidden rounded-lg bg-stone-900/20">
                      <StoryImage src={readerStory.image} alt="" fill className="object-cover" sizes="(max-width:768px) 100vw, 48vw" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/45 to-transparent" />
                    </div>
                    <h3 className="text-[clamp(1.15rem,2.2vw,1.6rem)] font-semibold leading-tight tracking-[-0.02em] text-white/[0.92]">
                      {readerStory.title}
                    </h3>
                    <p className="mt-3 text-[13px] leading-relaxed text-white/62">
                      This publisher blocked in-app embedding. Use the Original button to read the full story.
                    </p>
                  </div>
                )}
              </div>
